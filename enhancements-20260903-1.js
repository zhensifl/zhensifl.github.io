...
renderDashboard=()=>{
  const allDue=dueWords(),scheduled=allDue.slice(0,state.reviewLimit),completedNew=learnedToday(),fresh=Math.min(newAllowance(),unseenWords().length),weak=weakWords().length,total=scheduled.length+fresh,backlog=Math.max(0,allDue.length-scheduled.length);
...
window.WordStepEnhancements={version:"20260903-1",retrievability,nextSchedule,openPlacement,exportProgress,migrateLegacyMemoryState,vocabularyLevel};
