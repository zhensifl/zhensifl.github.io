...
const NEW_WORD_DAILY_MAX = 70;
const REVIEW_WORD_DAILY_MAX = 150;
...
const ACTIVITY_ACTOR=crypto.randomUUID();
function saveChangedSettings(previous){
  state.settingClocks={...(state.settingClocks||{})};
  const stamp=Math.max(Date.now(),...Object.values(state.settingClocks).map(value=>(Number(value)||0)+1));
  WORDSTEP_SETTINGS.forEach(key=>{if(state[key]!==previous[key])state.settingClocks[key]=stamp});
  saveState();
}
function placementCompletionKey(auth=authSession){return `wordstep-placement-complete:${auth?.user?.id||"anonymous"}`}
function hasStudyHistory(progress=state){return Object.values(progress.words||{}).some(record=>Number(record?.seen)>0||Number(record?.stage)>=0)}
function rememberPlacementCompletion(){try{localStorage.setItem(placementCompletionKey(),"1")}catch{}}
function ensurePlacementCompletion(){
  let remembered=false;try{remembered=localStorage.getItem(placementCompletionKey())==="1"}catch{}
  const completed=state.placementDone===true||remembered||hasStudyHistory();
  if(completed&&!state.placementDone){const previous={...state};state.placementDone=true;saveChangedSettings(previous)}
  if(completed)rememberPlacementCompletion();
  return completed;
}
...
function migrateLegacyMemoryState(source={}){
  ...
  migrated.settingsBaselineAt=Number(source.settingsBaselineAt??source.updatedAt)||0;
  migrated.settingClocks={...(source.settingClocks||{})};
  WORDSTEP_SETTINGS.forEach(key=>{if(migrated.settingClocks[key]===undefined){migrated.settingClocks[key]=0;changed=true}});
  ...
  const dailyGoal=Math.max(5,Math.min(NEW_WORD_DAILY_MAX,Number(migrated.dailyGoal)||10));
  const reviewLimit=Math.max(30,Math.min(REVIEW_WORD_DAILY_MAX,Number(migrated.reviewLimit)||50));
  if(dailyGoal!==Number(migrated.dailyGoal)||reviewLimit!==Number(migrated.reviewLimit))changed=true;
  migrated.dailyGoal=dailyGoal;migrated.reviewLimit=reviewLimit;
...
const CURATED_CEFR_LEVELS={
  laundry:"A2",neighbor:"A2",comfortable:"A2",prepare:"A2",repair:"A2",towel:"A2",blanket:"A2",order:"A2",menu:"A2",delicious:"A2",bill:"A2",expensive:"A2",receipt:"A2",change:"A2",available:"A2",
  station:"A2",ticket:"A2",direction:"A2",straight:"A2",corner:"A2",traffic:"A2",arrive:"A2",leave:"A2",nearby:"A2",entrance:"A2",delay:"A2",platform:"A2",introduce:"A2",invite:"A2",agree:"A2",probably:"A2",message:"A2",understand:"A2",explain:"A2",
  meeting:"A2",schedule:"A2",project:"A2",report:"A2",task:"A2",update:"A2",discuss:"A2",suggest:"A2",confirm:"A2",progress:"A2",problem:"A2",solution:"A2",support:"A2",improve:"A2",feedback:"A2",complete:"A2",expect:"A2",
  deadline:"B1",forward:"B1",attach:"B1",responsible:"B1",priority:"B1",decision:"B1",opportunity:"B1"
};
function vocabularyLevel(word){
  const en=word.en.toLowerCase(),rank=Number(word.frequencyRank)||Infinity;
  if(COMPACT_VOCABULARY&&WORDSTEP_LEVELS.includes(word.level))return word.level;
  if(Object.hasOwn(CURATED_CEFR_LEVELS,en))return CURATED_CEFR_LEVELS[en];
  const explicitLevel=word.cefrLevel;
  let level=explicitLevel||window.CEFRJ_LEVELS?.[en];
  if(level==="A1")level="A2";
  if(!level)level=rank<=5000?"B1":"B2";
  const abstractForm=/(?:tion|sion|ity|ness|ment|ance|ence|ism|ship|ology|graphy|ative|ous|ive)$/;
  const complexA2=en.length>=12||(en.length>=8&&abstractForm.test(en))||(en.length>=10&&/^(?:un|non|mis|over|under)/.test(en));
  const complexB1=en.length>=15||(Number.isFinite(rank)&&rank>=12000);
  if(!explicitLevel&&level==="A2"&&complexA2)level="B1";
  if(!explicitLevel&&level==="B1"&&complexB1)level="B2";
  return level;
}
WORDS.forEach((word,rank)=>{
  word.rank=rank;
  word.level=vocabularyLevel(word);
...
mergeProgress=(local,cloud)=>{
  ...
  const merged={...defaultState,words:{},activity:{},settingClocks:{},settingsBaselineAt:Math.max(a.settingsBaselineAt||0,b.settingsBaselineAt||0),memoryModelVersion:MEMORY_MODEL_VERSION,lastStudyDate:[a.lastStudyDate,b.lastStudyDate].filter(Boolean).sort().pop()||null,updatedAt:Math.max(a.updatedAt||0,b.updatedAt||0)};
  WORDSTEP_SETTINGS.forEach(key=>{merged[key]=mergeSetting(a,b,key,defaultState[key]);if(a.settingClocks?.[key]!==undefined||b.settingClocks?.[key]!==undefined)merged.settingClocks[key]=Math.max(Number(a.settingClocks?.[key])||0,Number(b.settingClocks?.[key])||0)});
  if(a.placementDone||b.placementDone){merged.placementDone=true;merged.settingClocks.placementDone=Math.max(Number(a.settingClocks?.placementDone)||0,Number(b.settingClocks?.placementDone)||0)}
  ...
  new Set([...Object.keys(a.activity),...Object.keys(b.activity)]).forEach(day=>{merged.activity[day]=mergeActivity(a.activity[day],b.activity[day],a.words,b.words,day)});
  return merged;
};
...
function reviewAllowance(){return Math.max(0,state.reviewLimit-Math.max(0,Number(state.activity[localDate()]?.reviews)||0))}
todayQueue=()=>[...dueWords().slice(0,reviewAllowance()),...unseenWords().slice(0,newAllowance())];
...
let placement={index:0,score:0,answers:[],knownIds:[]};
function placementWords(){
  return WORDSTEP_LEVELS.flatMap(level=>{const pool=WORDS.filter(word=>word.level===level),positions=[.08,.28,.55,.82];return positions.map(ratio=>pool[Math.min(pool.length-1,Math.floor(pool.length*ratio))]).filter(Boolean)});
}
function openPlacement(intro=true){
  ...
  if(intro){content.innerHTML=`<span class="eyebrow">PLACEMENT TEST · 水平测试</span><h2>从合适的难度开始</h2><p class="auth-lead">共 12 题，不需要猜。认识就选择含义，不认识直接跳过；答对的基础词会跳过初学，但仍会在以后复习。</p><div class="setting-note"><span>◎</span><div><strong>测试不会清除已有进度</strong><p>结果会推荐 A2、B1 或 B2，并减少重复学习已经掌握的基础词。</p></div></div><button class="primary-button full" id="begin-placement">开始测试</button><button class="text-button" id="skip-placement">暂不测试，从 A2 开始</button>`;document.querySelector("#begin-placement").onclick=()=>{placement={index:0,score:0,answers:[],knownIds:[]};renderPlacementQuestion()};document.querySelector("#skip-placement").onclick=()=>finishPlacement("A2");return}
...
function answerPlacement(correct){const word=placementWords()[placement.index];placement.score+=correct?1:0;placement.answers.push(correct);if(correct&&word)placement.knownIds.push(word.id);placement.index++;renderPlacementQuestion()}
...
function finishPlacement(level){
  const previous={...state},now=Date.now(),exact=new Set(placement.knownIds||[]),strongA2=(placement.answers||[]).slice(0,4).filter(Boolean).length>=3;
  WORDS.filter(word=>exact.has(word.id)||(strongA2&&word.isEssential)).forEach(word=>{if(ws(word.id).stage>=0)return;const confirmed=exact.has(word.id),stability=confirmed?30:7;state.words[word.id]={...ws(word.id),stage:confirmed?4:2,due:dayStart()+stability*DAY,stability,difficulty:confirmed?3:4,lastSeen:localDate(),learnedOn:null,lastReviewAt:now,lastRating:"easy",seen:1,correct:1,modifiedAt:now,placementKnown:true}});
  state.level=level;state.placementDone=true;rememberPlacementCompletion();saveChangedSettings(previous);setModalOpen("#placement-modal",false);populateEnhancedSettings();renderDashboard();renderLibrary();renderProgress();showToast(`已切换到 ${level}，以后只会在你主动重新测试时出现`)
}

function modeForQuestion(){if(state.trainingMode==="mixed")return session.idx%3===1?"listening":"reading";return state.trainingMode}
openSession=()=>{
  if(!ensurePlacementCompletion()){openPlacement(true);return}
  let queue=todayQueue(),isExtra=false;if(!queue.length&&reviewAllowance()>0){queue=weakWords().slice(0,Math.min(reviewAllowance(),Math.max(5,state.dailyGoal)));isExtra=true}if(!queue.length){showToast("今日计划已完成，或当前等级暂无新词；可在设置中查看学习计划");return}
  ...
  document.body.classList.add("session-open");document.querySelector("#session-overlay").classList.add("active");document.querySelector("#session-overlay").setAttribute("aria-hidden","false");renderQuestion();
};
...
renderQuestion=()=>{
  ...
  if(mode==="listening"){
    ...
    document.querySelectorAll(".choice").forEach(button=>button.addEventListener("click",()=>answerChoice(button,String(word.id),`${word.en} · ${meaningWithPartOfSpeech(word)}`,word.en,word.phonetic)));document.querySelector("#choice-unknown").onclick=()=>markChoiceUnknown(String(word.id),`${word.en} · ${meaningWithPartOfSpeech(word)}`,word.en,word.phonetic);session.listening++;setTimeout(()=>{if(session&&currentWord()?.id===word.id)speakWord(word.en)},100);return;
  }
...
choiceFeedback=({correct,answer,speakText,phonetic,unknown=false})=>{
  const example=exampleFor(currentWord());document.querySelector("#choice-feedback").innerHTML=`<div class="choice-example"><p>${escapeAttr(example.en)}</p><small>${escapeAttr(example.zh)}</small></div>${phraseHintHtml(currentWord())}<div class="feedback ${correct?"success":"error"}"><div><strong>${unknown?"已标记为不认识，正确答案：":correct?"答对了！":"记一下正确答案："} ${escapeAttr(answer)}</strong>${speakText?`<span class="phonetic feedback-phonetic">${escapeAttr(phonetic)}</span>`:""}</div><div class="feedback-actions">${speakText?`<button class="speak-button" data-speak="${escapeAttr(speakText)}" aria-label="朗读 ${escapeAttr(speakText)}">🔊</button>`:""}<button class="continue-button" id="to-spelling">继续拼写 →</button></div></div>`;document.querySelector("#to-spelling").onclick=()=>{session.phase="spelling";renderQuestion()};
};

renderSpelling=()=>{
  const word=currentWord(),example=clozeExample(word),dictation=session.currentQuestionType==="dictation";
  document.querySelector("#study-card").innerHTML=`<span class="question-type">${dictation?"DICTATION · 听写强化":"拼写练习 · 必须拼对才能继续"}</span>${dictation?`<div class="listening-prompt"><button class="speak-button large" data-speak="${escapeAttr(word.en)}" aria-label="播放单词发音">🔊</button><span class="mode-chip">听发音并拼写</span></div>`:`<h2 class="question-main chinese">${escapeAttr(primaryMeaning(word))}</h2><span class="phonetic">${escapeAttr(word.phonetic)}</span>`}<div class="spelling-example"><span>${example.kind==="definition"?"语境释义":"例句"}</span><p>${escapeAttr(example.en)}</p><small>${escapeAttr(example.zh)}</small></div>${phraseHintHtml(word,true)}<form class="spell-form" id="spell-form"><input class="spell-input" id="spell-input" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="输入英文单词" aria-label="输入英文拼写"><div class="spell-actions"><button class="primary-button" type="submit">检查拼写</button><button class="unknown-button" id="spell-unknown" type="button">不认识</button></div><div id="spell-feedback" aria-live="polite"></div></form>`;
  const input=document.querySelector("#spell-input");input.focus();document.querySelector("#spell-form").onsubmit=event=>{event.preventDefault();checkSpelling(input.value)};document.querySelector("#spell-unknown").onclick=markSpellingUnknown;if(dictation){session.listening++;setTimeout(()=>{if(session&&currentWord()?.id===word.id)speakWord(word.en)},100)}
};
...
finishWord=(rating="good")=>{
  ...
  const key=session.studyDate||localDate();recordActivity(state,key,ACTIVITY_ACTOR,word.id,wasNew,session.currentHadError,session.currentErrorCount,["listening","dictation"].includes(session.currentQuestionType),rating);state.words[word.id]={...record,stage:memoryStage(schedule.stability),due:dayStart()+schedule.interval*DAY,errors:(record.errors||0)+session.currentErrorCount,unknowns:unknownTotal,correct:(record.correct||0)+(!session.currentHadError?1:0),seen:(record.seen||0)+1,lastSeen:key,learnedOn:record.learnedOn||(wasNew?key:null),stability:schedule.stability,difficulty:schedule.difficulty,lapses:(record.lapses||0)+(rating==="again"?1:0),lastReviewAt:Date.now(),lastRating:rating,modifiedAt:Date.now()};
  state.lastStudyDate=key;saveState();
  session.correctWords=(session.correctWords||0)+(!session.currentHadError?1:0);session.idx++;session.phase="choice";session.currentHadError=false;session.currentErrorCount=0;session.currentUnknown=false;session.forceCorrection=false;session.currentQuestionType=null;if(session.idx>=session.queue.length)renderComplete();else renderQuestion();
};

renderComplete=()=>{const accuracy=session.queue.length?Math.round((session.correctWords||0)/session.queue.length*100):100;document.querySelector("#session-progress-bar").style.width="100%";document.querySelector("#session-step").textContent="完成";document.querySelector("#study-card").innerHTML=`<div class="completion-icon">✓</div><span class="question-type">TODAY COMPLETE</span><h2 class="question-main chinese">今天的训练完成了！</h2><p class="question-note">动态记忆模型已经根据本次答题表现安排好下一次复习。</p><div class="completion-stats"><div><strong>${session.queue.length}</strong><span>完成词数</span></div><div><strong>${accuracy}%</strong><span>本组准确率</span></div><div><strong>${session.listening}</strong><span>听力练习</span></div></div><button class="primary-button" id="finish-session">返回首页</button>`;document.querySelector("#finish-session").onclick=closeSession};

renderDashboard=()=>{
  ensurePlacementCompletion();
  const allDue=dueWords(),scheduled=allDue.slice(0,reviewAllowance()),completedNew=learnedToday(),fresh=Math.min(newAllowance(),unseenWords().length),weak=weakWords().length,total=scheduled.length+fresh,backlog=Math.max(0,allDue.length-scheduled.length);
  ...
  document.querySelector("#review-count").textContent=scheduled.length;document.querySelector("#new-count").textContent=fresh;document.querySelector("#new-completed").textContent=Math.min(completedNew,state.dailyGoal);document.querySelector("#new-goal").textContent=state.dailyGoal;document.querySelector("#weak-count").textContent=weak;
  document.querySelector("#task-total").textContent=`${total} 个待完成${backlog?` · ${backlog} 个积压已分批`:""}`;
  document.querySelector("#plan-summary").textContent=total?`先复习 ${scheduled.length} 词。今日新词目标 ${state.dailyGoal} 个，已完成 ${completedNew} 个，还需 ${fresh} 个，约需 ${Math.max(3,Math.ceil(total*.75))} 分钟。${backlog?`其余 ${backlog} 个复习会分批安排。`:""}`:"今天的计划已经完成。可以练习易错词或切换词汇等级。";
  const button=document.querySelector("#start-session");button.innerHTML=total?"开始今日学习 <span>→</span>":"再练一组易错词 <span>→</span>";
  const next=unseenWords()[0],preview=scheduled[0]||next;document.querySelector("#today-scene").textContent=next?`${next.level||state.level} · ${next.category}`:"当前等级已完成";renderPlanPreview(preview,scheduled.length?"待复习":"下一个新词");
  const activity=state.activity[localDate()]||{};document.querySelector("#review-progress").style.width=`${scheduled.length?Math.min(100,(activity.reviews||0)/(scheduled.length+(activity.reviews||0))*100):100}%`;document.querySelector("#new-progress").style.width=`${state.dailyGoal?Math.min(100,completedNew/state.dailyGoal*100):0}%`;document.querySelector("#weak-progress").style.width=`${weak?Math.min(100,(activity.spelling||0)/weak*100):100}%`;document.querySelector("#placement-banner").hidden=state.placementDone;renderWeek();updateConnectionStatus();
};

let activeLevelFilter="全部";
renderLibrary=()=>{
  document.querySelector("#category-filters").innerHTML=["全部",...categoryNames].map(category=>`<button class="filter-button ${category===activeCategory?"active":""}" data-category="${category}">${category}</button>`).join("");
  const learned=WORDS.filter(word=>ws(word.id).stage>=0).length,mastered=WORDS.filter(word=>ws(word.id).stability>=30).length,search=document.querySelector("#word-search").value.trim().toLowerCase(),filtered=WORDS.filter(word=>(activeCategory==="全部"||word.category===activeCategory)&&(activeLevelFilter==="全部"||word.level===activeLevelFilter)&&(!search||word.en.includes(search)||word.zh.includes(search))),visible=filtered.slice(0,libraryLimit),hasFilter=activeCategory!=="全部"||activeLevelFilter!=="全部"||!!search;
  document.querySelector("#library-stats").innerHTML=`<span class="small-stat total-stat"><small>完整词库 · v${APP_VERSION}</small><strong>${WORDS.length.toLocaleString("zh-CN")}</strong><b>词</b></span><span class="small-stat visible-stat"><small>当前筛选</small><strong>${filtered.length.toLocaleString("zh-CN")}</strong><b>词</b></span><span class="small-stat"><small>页面已显示</small><strong>${visible.length}</strong><b>词</b></span>${WORDSTEP_LEVELS.map(level=>`<span class="small-stat level-stat"><strong>${WORDS.filter(word=>word.level===level).length.toLocaleString("zh-CN")}</strong> ${level}</span>`).join("")}<span class="small-stat level-stat"><strong>${learned}</strong> 已学习</span><span class="small-stat level-stat"><strong>${mastered}</strong> 稳定掌握</span>${hasFilter?`<button class="small-stat stats-reset" id="clear-library-filters">清除筛选，查看全部</button>`:""}`;
  document.querySelector("#word-list").innerHTML=filtered.length?visible.map(word=>{const record=ws(word.id),example=exampleFor(word),status=record.stage<0?["未学习",""]:record.stability>=30?["已掌握","mastered"]:["学习中","learning"];return `<div class="word-row"><div class="word-en"><div class="word-title"><strong>${escapeAttr(word.en)}</strong><span class="level-badge">${word.level}</span><button class="speak-button" data-speak="${escapeAttr(word.en)}" aria-label="朗读 ${escapeAttr(word.en)}">🔊</button></div><small>${escapeAttr(word.phonetic)}</small></div><div class="word-zh">${escapeAttr(word.zh)}</div><div class="word-example">${escapeAttr(example.en)}<br><small>${escapeAttr(example.zh)}</small></div><span class="status-badge ${status[1]}">${status[0]}</span></div>`}).join("")+`${visible.length<filtered.length?`<button class="load-more" id="load-more">继续显示 ${Math.min(100,filtered.length-visible.length)} 个（当前 ${visible.length} / 共 ${filtered.length}）</button>`:""}`:`<div class="empty-state">没有找到匹配的单词</div>`;
};
...
updateRange=()=>{const count=+document.querySelector("#daily-range").value;document.querySelector("#daily-value").textContent=count;document.querySelector("#estimated-time").textContent=Math.ceil((count+Math.min(reviewAllowance(),dueWords().length))*.72)};
...
document.querySelector("#save-settings").onclick=()=>{const previous={...state};state.dailyGoal=+document.querySelector("#daily-range").value;state.level=document.querySelector("#level-select").value;state.track=document.querySelector("#track-select").value;state.trainingMode=document.querySelector("#mode-select").value;state.reviewLimit=+document.querySelector("#review-limit-select").value;saveChangedSettings(previous);setModalOpen("#settings-modal",false);renderDashboard();renderLibrary();renderProgress();showToast("学习计划与训练模式已更新")};
...
document.querySelector("#library-stats").addEventListener("click",event=>{if(event.target.id!=="clear-library-filters")return;activeCategory="全部";activeLevelFilter="全部";libraryLimit=100;document.querySelector("#word-search").value="";document.querySelector("#library-level-filter").value="全部";renderLibrary()});
...
async function consumeRecoveryLink(){const params=new URLSearchParams(location.hash.slice(1));if(params.get("type")!=="recovery"||!params.get("access_token"))return false;try{const user=await supabaseRequest("/auth/v1/user",{token:params.get("access_token")});activateAccount({access_token:params.get("access_token"),refresh_token:params.get("refresh_token"),expires_at:Math.floor(Date.now()/1000)+Number(params.get("expires_in")||3600),user});authMode="recovery";history.replaceState(null,"",location.pathname+location.search);renderAuthModal();setModalOpen("#auth-modal",true);return true}catch(error){authMessage=authErrorText(error.message);return false}}
...
syncProgress=async(notify=false)=>{
  if(!authSession)return false;if(syncBusy){syncQueued=true;return false}
  const epoch=authEpoch,userId=authSession.user.id,isCurrent=()=>epoch===authEpoch&&authSession?.user?.id===userId;
  syncBusy=true;const replaceCloud=replaceCloudOnNextSync;replaceCloudOnNextSync=false;syncStatus="正在同步…";renderAccountUI();
  try{
    const valid=await ensureSession();if(!isCurrent())return false;if(!valid)throw new Error("暂时无法刷新登录，请检查网络后重试");
    const rows=await supabaseRequest('/rest/v1/user_progress?select=state&user_id=eq.'+encodeURIComponent(userId),{token:valid.access_token});
    if(!isCurrent())return false;
    if(!replaceCloud)state=mergeProgress(state,rows?.[0]?.state);persistLocalState();
    await supabaseRequest("/rest/v1/user_progress?on_conflict=user_id",{method:"POST",body:{user_id:userId,state:structuredClone(state)},token:valid.access_token,headers:{Prefer:"resolution=merge-duplicates"}});
    if(!isCurrent())return false;
    lastSyncError="";syncStatus="已同步 · "+new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit"});renderDashboard();renderLibrary();renderProgress();if(notify)showToast("学习进度已同步");return true;
  }catch(error){
    if(!isCurrent())return false;
    if(replaceCloud)replaceCloudOnNextSync=true;lastSyncError=authErrorText(error.message);syncStatus="同步失败 · "+lastSyncError;clearTimeout(syncTimer);if(authSession)syncTimer=setTimeout(()=>syncProgress(),15000);if(notify)showToast(syncStatus);return false;
  }finally{syncBusy=false;renderAccountUI();updateConnectionStatus();if(syncQueued){syncQueued=false;scheduleCloudSync()}}
};
...
ensurePlacementCompletion();populateEnhancedSettings();renderDashboard();renderLibrary();renderProgress();consumeRecoveryLink();setTimeout(()=>{if(!ensurePlacementCompletion())openPlacement(true)},700);
window.WordStepEnhancements={version:"20260907-8",retrievability,nextSchedule,openPlacement,exportProgress,migrateLegacyMemoryState,vocabularyLevel,ensurePlacementCompletion};
