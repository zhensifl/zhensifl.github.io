...
renderLibrary=()=>{
  ...
  document.querySelector("#library-stats").innerHTML=`<span class="small-stat total-stat"><small>完整词库</small><strong>${WORDS.length.toLocaleString("zh-CN")}</strong><b>词</b></span><span class="small-stat visible-stat"><small>当前筛选</small><strong>${filtered.length.toLocaleString("zh-CN")}</strong><b>词</b></span><span class="small-stat"><small>页面已显示</small><strong>${visible.length}</strong><b>词</b></span>${WORDSTEP_LEVELS.map(level=>`<span class="small-stat level-stat"><strong>${WORDS.filter(word=>word.level===level).length.toLocaleString("zh-CN")}</strong> ${level}</span>`).join("")}<span class="small-stat level-stat"><strong>${learned}</strong> 已学习</span><span class="small-stat level-stat"><strong>${mastered}</strong> 稳定掌握</span>${hasFilter?`<button class="small-stat stats-reset" id="clear-library-filters">清除筛选，查看全部</button>`:""}`;
...
window.WordStepEnhancements={version:"20260907-5",retrievability,nextSchedule,openPlacement,exportProgress,migrateLegacyMemoryState,vocabularyLevel};
