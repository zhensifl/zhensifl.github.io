...
const BASE_WORDS = [...CURATED_WORDS,...(window.WORD_DATA||[])].map((word,index)=>{const essential=ESSENTIAL_BY_EN.get(word.en.toLowerCase()),merged=essential?{...word,...essential}:word;return {...merged,pos:partOfSpeech(merged),id:index+1,isCurated:index<CURATED_WORDS.length||!!essential,isEssential:!!essential}});
const BASE_WORD_KEYS=new Set(BASE_WORDS.map(word=>word.en.toLowerCase()));
const ALL_WORDS = [...BASE_WORDS,...(window.WORDSTEP_ESSENTIAL_WORDS||[]).filter(word=>!BASE_WORD_KEYS.has(word.en.toLowerCase())).map((word,index)=>({...word,pos:partOfSpeech(word),id:BASE_WORDS.length+index+1,isCurated:true,isEssential:true}))];
...
async function verifyContentBundle(){
  const focusVersion=window.WORDSTEP_FOCUS_INFO?.version,dailyVersion=window.WORDSTEP_DAILY_COMMUNICATION_INFO?.version;
  if(WORDS.length>=5000&&WORDS.length<=6000&&focusVersion==="20260907-2"&&dailyVersion==="20260907-4")return true;
